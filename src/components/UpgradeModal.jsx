...
export default function UpgradeModal({ isOpen, onClose }) {
  ...
  const benefits = t('app.upgrade.benefits', { returnObjects: true })
    .map((text, i) => ({ icon: BENEFIT_ICONS[i], text }));
        ...
        >
          ...
          >
            ...
            <div style={{
              ...
              <div style={{ fontSize: '2rem', fontWeight: 800, color: '#92400e' }}>{t('app.upgrade.price')}<span style={{ fontSize: '1rem', fontWeight: 500 }}>{t('app.upgrade.period')}</span></div>
            ...
            >
              {isLoading ? t('app.upgrade.ctaLoading') : t('app.upgrade.cta')}
            </button>

            <p style={{ textAlign: 'center', fontSize: '0.75rem', color: '#9ca3af', marginTop: '0.75rem' }}>
              {t('app.upgrade.footer')}
            </p>
          </motion.div>
        </motion.div>
      )}
